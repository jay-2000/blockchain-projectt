module.service("messageFactory", [
    "$cookies",
    "$q",
    "$rootScope",
    function ($cookies, $q, $rootScope) {
        let vm = this;
        ...
        if (typeof web3 !== "undefined") {
            ...
            // Edit account id here
            let EthChat = EthChatContract.at(
                "0x50C16ebf49db60fBB3A7Ca3f0fF2630b420200Fa"
            );
            ...
            let fetchMessagesForRoom = function (roomName) {
                ...
                EthChat.getMessageCountForRoom(
                    roomName,
                    { from: web3.eth.defaultAccount },
                    (err, result) => {
                        if (err) {
                            console.log(
                                "Error fetching message count for room " +
                                    chatRoom
                            );
                            console.log(err);
                            deferred.reject(err);
                            ...
                            if (messageCount === 0) {
                                ...
                                for (let i = 0; i < messageCount; i++) {
                                    EthChat.getMessageByIndexForRoom(
                                        roomName,
                                        i,
                                        { from: web3.eth.defaultAccount },
                                        function (err, result) {
            ...
            let getMessagesForRoom = function (roomName) {
                ...
                if (chatRoomMessages[roomName]) {
                    console.log(
                        "Loading from cache: " +
                            JSON.stringify(chatRoomMessages[roomName])
                    );
            ...
            event.watch(function (error, result) {
                if (error)
                    console.log(
                        "wait for a while, check for block Synchronization or block creation, " +
                            error
                    );
                ...
                if (
                    !newMessages[room] ||
                    !newMessages[room].includes(result.transactionHash)
                ) {
                    if (newMessages[room]) {
